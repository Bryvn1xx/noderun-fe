...
const WindowsPostPage = () => {
  ...
  const getPost = async () => {
    try {
      const post = await axios.get(`https://noderun-be.herokuapp.com/api/posts/${postId}/2`)
  ...
  const getCommentsByPost = async () => {
    try {
      const comments = await axios.get(`https://noderun-be.herokuapp.com/api/comments/${postId}`)
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await axios.post(
      `https://noderun-be.herokuapp.com/api/comments/${postId}`,
      addComment
    )
